...
public class EmergencyFundInternalRestorationTests
{
    ...
    public async Task UseAndRestoration_UpdateOnlyGoalData()
    {
        ...
        var goal = new SavingsGoal { Id = Guid.NewGuid(), UserId = user.Id, Name = "Fondo", Purpose = SavingsGoalPurpose.EmergencyFund, TargetAmount = 1_000m, CurrentAmount = 1_000m, MinimumProtectedAmount = 500m };
        context.AddRange(user, cash, goal);
        await context.SaveChangesAsync();

        var unitOfWork = new UnitOfWork(context);
        var service = new EmergencyFundRestorationService(new SavingsGoalRepository(context), new EmergencyFundRestorationRepository(context), unitOfWork);
        ...
        Assert.Equal(1_500m, (await context.FinancialAccounts.SingleAsync()).CurrentBalance);
