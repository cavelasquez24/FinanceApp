  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-finflow-dark/40 p-4 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="w-full max-w-sm rounded-[28px] border border-[#EFEAE2] bg-white/90 p-6 sm:p-8 shadow-xl backdrop-blur-xl animate-in zoom-in-95 duration-200 text-center">
        <div className="mx-auto flex h-14 w-14 items-center justify-center rounded-full bg-finflow-rust/10 mb-4">
          <AlertTriangle className="h-7 w-7 text-finflow-rust" />
        </div>
        
        <h2 className="font-serif text-xl font-semibold text-finflow-dark mb-2">
          ¿Eliminar Categoría?
        </h2>
        ...
        <div className="flex flex-col gap-3">
          <Button
            ...
            className="w-full h-11 rounded-xl !bg-finflow-rust text-sm font-medium !text-white shadow-sm transition-all hover:!bg-[#A6604D] disabled:opacity-70"
          >
          ...
          <Button
            ...
            onClick={onClose}
            disabled={deleteMutation.isPending}
            className="w-full h-11 rounded-xl text-sm font-medium text-finflow-muted hover:bg-[#EFEAE2]/60 transition-colors"
          >
            Cancelar
          </Button>
        </div>
