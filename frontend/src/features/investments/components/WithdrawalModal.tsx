...
export function WithdrawalModal({ investment, onSuccess, onCancel }: Props) {
  ...
  const { register, handleSubmit, control, formState: { errors } } = useForm<WithdrawalFormValues>({
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
      ...
      <div className="rounded-2xl bg-[#FBF9F4] border border-[#EFEAE2] p-4 space-y-1 text-sm">
        <p className="font-semibold text-[#2C2A29]">{investment.name}</p>
        <div className="flex gap-6 text-[#7C756E]">
          <span>Capital base: <span className="font-medium text-[#2C2A29]">{formatCurrency(investment.contributedCapital)}</span></span>
          <span>Valor actual: <span className="font-medium text-[#2C2A29]">{formatCurrency(investment.currentValue)}</span></span>
        </div>
      </div>
      ...
      {safeAmount > 0 && (
        <div className="rounded-2xl border border-[#EFEAE2] bg-white/60 p-4 space-y-2 text-sm">
          <p className="text-xs font-semibold text-[#7C756E] uppercase tracking-wider mb-3">Resumen del Retiro</p>
          <div className="flex justify-between">
            <span className="text-[#7C756E]">Monto bruto</span>
            <span className="font-medium text-[#2C2A29]">{formatCurrency(safeAmount)}</span>
          </div>
          {safeFee > 0 && (
            <div className="flex justify-between">
              <span className="text-[#7C756E]">Comisión</span>
              <span className="font-medium text-[#C97B63]">–{formatCurrency(safeFee)}</span>
            </div>
          )}
          <div className="flex justify-between border-t border-[#EFEAE2] pt-2">
            <span className="text-[#7C756E]">Efectivo neto recibido</span>
            <span className="font-bold text-[#2C2A29]">{formatCurrency(netCash)}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-[#7C756E]">Ganancia/Pérdida realizada</span>
            <span className={`font-bold ${realizedGain >= 0 ? 'text-[#8FA888]' : 'text-[#C97B63]'}`}>
              {realizedGain >= 0 ? '+' : ''}{formatCurrency(realizedGain)}
            </span>
          </div>
          <div className="border-t border-[#EFEAE2] pt-2 mt-1 space-y-1 text-xs text-[#7C756E]">
            <div className="flex justify-between">
              <span>Capital base restante</span>
              <span className={remainingCapital < 0 ? 'text-[#C97B63] font-medium' : ''}>{formatCurrency(Math.max(0, remainingCapital))}</span>
            </div>
            <div className="flex justify-between">
              <span>Valor de posición restante</span>
              <span className={remainingValue < 0 ? 'text-[#C97B63] font-medium' : ''}>{formatCurrency(Math.max(0, remainingValue))}</span>
            </div>
          </div>
        </div>
      )}
