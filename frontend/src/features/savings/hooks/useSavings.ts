...
export function useCreateSavingsGoal() {
  ...
  return useMutation({
    ...
    onError: (error) =>
      toast.error(
        getApiErrorMessage(error, "Error al crear la meta de ahorro"),
      ),
  });
}
...
export function useUpdateSavingsGoal() {
  ...
  return useMutation({
    ...
    onError: (error) =>
      toast.error(getApiErrorMessage(error, "No se pudo actualizar la meta")),
  });
}
...
export function useDeleteSavingsGoal() {
  ...
  return useMutation({
    mutationFn: ({ id, data }: { id: string; data?: ArchiveSavingsGoalDto }) =>
      savingsApi.delete(id, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["savings-goals"] });
      queryClient.invalidateQueries({ queryKey: ["dashboard"] });
    },
    onError: (error) =>
      toast.error(getApiErrorMessage(error, "No se pudo archivar la meta")),
  });
}
  ...
  return useMutation({
    ...
    onError: (error: ApiError) => {
      ...
      if (errorCode === "GOAL_ALREADY_COMPLETED") {
        ...
        toast.error(
          getApiErrorMessage(error, "Error al registrar el depósito"),
        );
      }
    },
  });
...
export function useWithdrawSavings() {
  ...
  return useMutation({
    ...
    onError: (error: ApiError) => {
      const errorCode = error.response?.data?.error?.code;
      if (errorCode === "INSUFFICIENT_SAVINGS_BALANCE") {
