...
export const savingsApi = {
  ...
  getAll: async () => {
    const response =
      await apiClient.get<ApiResponse<SavingsGoal[]>>("/savings-goals");
    return response.data.data;
  },

  // GET /api/v1/savings-goals/{id}
  getById: async (id: string) => {
    const response = await apiClient.get<ApiResponse<SavingsGoal>>(
      `/savings-goals/${id}`,
    );
    return response.data.data;
  },
  ...
  update: async (id: string, data: Partial<CreateSavingsGoalDto>) => {
    const response = await apiClient.put<ApiResponse<SavingsGoal>>(
      `/savings-goals/${id}`,
      data,
    );
    return response.data.data;
  ...
  withdraw: async (id: string, data: WithdrawDto) => {
    const response = await apiClient.post<ApiResponse<SavingsGoalWithdrawal>>(
  ...
  getRestorations: async (goalId: string) => {
    const response = await apiClient.get<
      ApiResponse<EmergencyFundRestoration[]>
    >(`/savings-goals/${goalId}/restorations`);
    return response.data.data;
  },
  ...
  createEmergencyFundUse: async (goalId: string, data: EmergencyFundUseDto) => {
    const response = await apiClient.post<
      ApiResponse<EmergencyFundRestoration>
    >(`/savings-goals/${goalId}/emergency-fund-uses`, data);
    return response.data.data;
  },

  registerRestorationPayment: async (
    restorationId: string,
    data: EmergencyFundRestorationPaymentDto,
  ) => {
    const response = await apiClient.post<
      ApiResponse<EmergencyFundRestoration>
    >(`/emergency-fund-restorations/${restorationId}/payments`, data);
    return response.data.data;
  },
  ...
  cancelRestoration: async (restorationId: string) => {
    const response = await apiClient.post<
      ApiResponse<EmergencyFundRestoration>
    >(`/emergency-fund-restorations/${restorationId}/cancel`);
    return response.data.data;
  },
};
